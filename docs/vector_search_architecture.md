...
- **Core API (FastAPI)**: Responsible for handling standard CRUD requests and Search Requests.
- **Database (PostgreSQL + pgvector)**: Stores both structured data (Relational Data) and unstructured data (Vector Embeddings), and handles Triggers & Notify events.
- **Background Worker (Asyncpg Listener)**: A background process that listens to database events to generate Vector Embeddings asynchronously.
...
LIMIT $2
```
*(Note: The `<=>` operator returns Cosine Distance. Subtracting the Distance from 1 (`1 - Distance`) yields the Similarity Score).*
